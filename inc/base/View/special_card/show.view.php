				...
				<div class="container-fluid">
					<div class="row justify-content-center align-items-center">
						<div class="col-lg-5 img-video-content">
							<video id="my-video" class="video-js" controls poster="<?= wp_get_attachment_image_url( get_field( 'special_card_single_intro_video_poster' ), 'large' ) ?>" data-setup='' loop>
								<source src="<?= get_field( 'special_card_single_intro_video_url' ) ?>" type='video/mp4'>
							</video>
						</div>
						<div class="col-lg-6 col-md-12 text-content">
							<h2 class="color-7B5  mb-4">
								<?= get_field( 'special_card_single_intro_title' ) ?>
							</h2>
							<span class="color-C89 d-block mb-4">
								<?= get_field( 'special_card_single_intro_subtitle' ) ?>
							</span>
							<p class="text-justify color-313 mb-4">
								<?= nl2br( get_field( 'special_card_single_description' ) ) ?>
							</p>
							<div class="items-about">
								<?php if ( $features = get_field( 'special_card_single_special_card_features' ) ): ?>
					...
					<div class="row justify-content-center align-items-center">
						...
						<div class="col-lg-6  col-md-12 text-center form-content  mt-4  ">
							<form action="">
								...
								<div class="alert alert-danger special-card-alert" style="display: none"></div>
								<input type="hidden" id="special-card-id" value="<?= get_the_ID() ?>">
								<button class="btn-style-01 mt-4 btn-send-special_card-request">
									ثبت نام
								</button>
							</form>
						</div>
...
<?php add_action( 'theme_view_scripts', function () {
	...
	<script>
        ...
        $( document ).ready( function () {
            ...
            $( '.btn-send-special_card-request' ).click( function () {
                var _self = $(this);
                if ( is_loading === false ) {
                    is_loading = true;
                    _self.text( 'لطفا صبر کنید...' );
                    send_post_api( '<?= site_url( 'api/special_card_request' ) ?>', {
                        'captcha': $( '#captcha' ).val(),
                        'mobile': $( '#mobile-number' ).val(),
                        'user_name': $( '#username' ).val(),
                        'gender': $( '#gender' ).val(),
                        'city': $( '#city' ).val(),
                        'special_card_id': $( '#special-card-id' ).val(),
                    }, function ( result ) {
